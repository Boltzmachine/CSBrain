...
class CustomDataset(Dataset):
    """Forenzo2024 Continuous-Pursuit dataset -- REGRESSION view.

    Same EEG samples/LMDB as the classification view, but the target is the
    2D cursor VELOCITY (cursorvel x,y) averaged over the 1-second window
    (stored per record under 'vel'). Consumed by train_for_regression_dict
    with an MSE loss and corr/r2/rmse metrics.
    """
    ...
    def __getitem__(self, idx):
        ...
        return {
            'x': pair['sample'] / 100,
            'y': np.asarray(pair['vel'], dtype=np.float32),   # (2,) cursor velocity
    ...
    def collate(self, batch):
        ...
        y_label = np.array([x['y'] for x in batch], dtype=np.float32)
        ch_coords = np.array([x['ch_coords'] for x in batch])
        return {
            'x': to_tensor(x_data),
            'y': to_tensor(y_label).float(),                  # (B, 2)
...
class LoadDataset(object):
    ...
    def get_data_loader(self):
        ...
        print(len(train_set) + len(val_set) + len(test_set))
        # Honor --num_workers (0 avoids the LMDB-fork segfault on the ~401k-entry
        # Forenzo env; see forenzo_dataset).
