...
class CausalAdjacency(nn.Module):
    """
    Generate causal dependency adjacency matrix based on channel's positional encoding
    """
    ...
    def __init__(self, input_dim, hidden_dim=128):
        ...
        C = 19
        T = 30
        self.adjacency = nn.Parameter(torch.zeros(T-1, C, C), requires_grad=True) # lag, C, C
        nn.init.xavier_uniform_(self.adjacency)
    
    def forward(self, coord_pe, hidden_emb):
        """
        coord_pe: (C, d)
        hidden_emb: (B, C, N, d)
        """
        B, C, N, d = hidden_emb.shape
        # hidden = self.mlp(coord_pe)  # (C, hidden_dim)
        # adjacency = torch.matmul(hidden, hidden.transpose(0, 1))  # (C, C)
        
        # # Create causal mask (lower triangular)
        adjacency = self.adjacency
        adjacency = F.softmax(adjacency, dim=-1)  # Normalize to get attention weights

        num_lags = adjacency.size(0)

        causal_loss = 0.0
        # present_emb: (B, C, N, d)
        for lag in range(1, num_lags + 1):
            # Align current time with lagged past: (B, L, C, d), L = N - lag
            cur = hidden_emb[:, :, lag:, :].permute(0, 2, 1, 3) # (B, N-lag, C, d)
            past = hidden_emb[:, :, :-lag, :].permute(0, 2, 1, 3) # (B, N-lag, C, d)

            # (i, j): channel i at current time depends on channel j at t-lag
            # (B, N-lag, C, d), (B, N-lag, C, d) -> (B, N-lag, C, C), between 0 and 1
            cos_sim = F.cosine_similarity(cur.unsqueeze(2), past.unsqueeze(3), dim=-1)  # (B, N-lag, C, C)
            adj = adjacency[lag-1].softmax(dim=-1)  # (C, C)
            causal_loss += ((1 - cos_sim) * adj).mean()

        return causal_loss



class ChannelCausal(nn.Module):
    def __init__(self, in_dim=200, out_dim=200, d_model=200, dim_feedforward=800, seq_len=30, n_layer=12,
        ...
        reghead_layer = CSBrain_TransformerEncoderLayer(
            d_model=d_model, nhead=nhead, dim_feedforward=dim_feedforward, area_config=self.area_config, sorted_indices=self.sorted_indices, batch_first=True,
            activation=F.gelu
        )
        self.reghead_encoder = CSBrain_TransformerEncoder(reghead_layer, num_layers=1, enable_nested_tensor=False)
        ...
        self.causal_adjacency = CausalAdjacency(input_dim=3 * 2 * self.spherical_num_freqs, hidden_dim=d_model)
    ...
    def forward(self, x, mask=None):
        ...
        # reg_emb = self.reghead_encoder(patch_emb, self.area_config)
        # channel_causal_loss = self.causal_adjacency(None, reg_emb)

        out = self.proj_out(patch_emb)

        return {
            "out": out,
            # "channel_causal_loss": channel_causal_loss,
        }
